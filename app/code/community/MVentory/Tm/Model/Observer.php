...
class MVentory_Tm_Model_Observer
{
    ...
    public function productSaveAfter($observer)
    {
        ...
        $product = Mage::getModel('catalog/product')->load($event->getProduct()->getId());
        $stock = Mage::getModel('cataloginventory/stock_item')->loadByProduct($product);
        
        $mventoryCategoryId = Mage::app()->getRequest()->getParam('mventory_category');
        ...
        if ($stock->getManageStock() && $stock->getQty() == 0 && $product->getMventoryTmId()) {
            ...
            if ($result === true) {
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('catalog')->__('Removed!'));

...
class MVentory_Tm_Model_Connector extends Mage_Core_Model_Abstract
{
    ...
    {
        $this->_host = Mage::getStoreConfig(self::SANDBOX_PATH) ? 'tmsandbox' : 'trademe';
        ...
        $this->_config = array(
            ...
            'requestTokenUrl' => 'https://secure.' .$this->_host . '.co.nz/Oauth/RequestToken',
            'userAuthorisationUrl' => 'https://secure.' .$this->_host . '.co.nz/Oauth/Authorize',
            'accessTokenUrl' => 'https://secure.' .$this->_host . '.co.nz/Oauth/AccessToken',
    ...
    public function auth()
    {
        ...
        if (!$accessTokenData) {
            ...
            if (Mage::app()->getRequest()->getParam('oauth_token') && $session->getMventoryTmRequestToken()) {
                try {
                    $accessToken = $oAuth->getAccessToken(Mage::app()->getRequest()->getParams(), unserialize($session->getMventoryTmRequestToken()));

                    $accessTokenData = serialize(array($accessToken->getToken(), $accessToken->getTokenSecret()));

                    Mage::getConfig()->saveConfig(self::ACCESS_TOKEN_PATH, $accessTokenData);
                    Mage::getConfig()->reinit();
                    Mage::app()->reinitStores();
            ...
            } elseif (Mage::app()->getRequest()->getParam('denied')) {
                return false;
            } else {
                try {
                    ...
                    Mage::app()->getResponse()->setRedirect('https://secure.' . $this->_host . '.co.nz/Oauth/Authorize?scope=MyTradeMeRead,MyTradeMeWrite&oauth_token=' . $requestToken[1]);
    ...
    public function send($product) {
        ...
        if ($accessTokenData = $this->auth()) {
            ...
            $imagePath = Mage::getBaseDir('media') . DS . 'catalog' . DS . 'product' . $product->getImage();
            
            if (file_exists($imagePath)) {
                $imagePathInfo = pathinfo($imagePath);

                $signature = md5(Mage::getStoreConfig(self::KEY_PATH) . $imagePathInfo['filename'] . $imagePathInfo['extension'] . 'False' . 'False');
